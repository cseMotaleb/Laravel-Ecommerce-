					...
					<form class="form-horizontal" action="{{ url('/save-product') }}" method="POST" enctype="multipart/form-data">
					  ...
					  <fieldset>
						<div class="control-group">
						  ...
						  <div class="controls">
							<input type="text"  name="product_name" class="input-xlarge" id="date01" required="">
						  </div>
						</div>
						<div class="control-group">
							   ...
							   <select id="selectError3" name="category_id">
								<option>Select Product</option>
		                         ...
		                         $all_published_category=DB::table('tbl_category')
		                                                  ->where('pub_status',1)
		                                                  ->get();
		                            foreach($all_published_category as $cat_category){?>  
								<option value="{{$cat_category->category_id}}">{{$cat_category->category_name}}</option>	
								<?php } ?>		
							 	</select>
							</div>
						 </div> 
							  ...
							  <select id="selectError3" name="brand_id">
				                ...
				                <?php
		                           $all_published_brand=DB::table('tbl_brand')
		                                                  ->where('pub_status', 1)
		                                                  ->get();
		                            foreach($all_published_brand as $cat_brand){?>  
					...
					<div class="control-group">
					  ...
					  <div class="controls">
						<input type="text" class="input-xlarge" name="product_color" required="">
							...
							<div class="control-group hidden-phone">
							  <label class="control-label" for=" checkin">Published Status</label>
							  <div class="controls">
